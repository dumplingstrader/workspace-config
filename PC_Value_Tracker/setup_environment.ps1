...
$dirs = @("data", "data/raw", "output", "output/monthly", "output/quarterly", "output/scorecards", "templates", "submissions")
foreach ($dir in $dirs) {
...
if (-not (Test-Path "data/master.json")) {
...
"@
    $emptyData | Out-File -FilePath "data/master.json" -Encoding UTF8
    Write-Host "  Created empty data/master.json" -ForegroundColor Green
    Write-Host "  Ready for data collection!" -ForegroundColor Gray
} else {
    $json = Get-Content "data/master.json" -Raw | ConvertFrom-Json
    $count = $json.metadata.total_entries
    Write-Host "  data/master.json exists ($count entries)" -ForegroundColor Gray
}
...
if ($allGood) {
    ...
    Write-Host ""
    Write-Host "  3. To activate environment in future sessions:" -ForegroundColor Yellow
    Write-Host "     .\.venv\Scripts\activate" -ForegroundColor Cyan
} else {
    Write-Host "SETUP INCOMPLETE" -ForegroundColor Red
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Some packages failed to install. Try manually:" -ForegroundColor Yellow
