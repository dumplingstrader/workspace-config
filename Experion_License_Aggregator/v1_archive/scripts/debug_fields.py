"""Check actual license data after merge."""
import json
import csv
from pathlib import Path
from xml_parser import parse_all_licenses

# Load licenses
data_dir = Path('../data/raw')
clusters = ['Carson']
exclude = []

licenses, errors = parse_all_licenses(data_dir, clusters, exclude)

# Load utilization
utilization = {}
util_path = Path('../data/utilization_input.csv')
with open(util_path, 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        key = (row['cluster'], row['msid'], row['system_number'])
        utilization[key] = row

# Merge
for lic in licenses:
    cluster = lic.get('cluster', '')
    msid = lic.get('msid', '')
    system_number = lic.get('system_number', '')
    key = (cluster, msid, system_number)
    
    if key in utilization:
        util = utilization[key]
        for field, value in util.items():
            if field not in ['cluster', 'msid', 'system_number', 'as_of_date']:
                try:
                    lic[f'{field.upper()}_USED'] = int(value) if value else 0
                except:
                    lic[f'{field.upper()}_USED'] = 0

# Find M0921
m0921 = None
for lic in licenses:
    if lic.get('msid') == 'M0921' and lic.get('system_number') == '50216':
        m0921 = lic
        break

if m0921:
    print('M0921 Fields:')
    print(f'  cluster: {m0921.get("cluster")}')
    print(f'  msid: {m0921.get("msid")}')
    print(f'  system_number: {m0921.get("system_number")}')
    print(f'  PROCESSPOINTS (from XML): {m0921.get("PROCESSPOINTS")}')
    print(f'  PROCESSPOINTS_USED (from CSV): {m0921.get("PROCESSPOINTS_USED")}')
    print(f'  STATIONS (from XML): {m0921.get("STATIONS")}')
    print(f'  STATIONS_USED (from CSV): {m0921.get("STATIONS_USED")}')
    print()
    
    # Check all uppercase fields
    print('All uppercase license fields:')
    for key in sorted(m0921.keys()):
        if key.isupper():
            print(f'  {key}: {m0921[key]}')
else:
    print('M0921 not found')
