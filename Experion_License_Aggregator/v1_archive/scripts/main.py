...
def load_utilization_data(utilization_file: str = '../data/utilization_input.csv') -> dict:
    """Load utilization data from CSV."""
    utilization = {}
    util_path = Path(utilization_file)
    ...
    try:
        ...
        print(f"Warning: Could not load utilization data: {e}")
    
    return utilization


def merge_utilization_data(licenses: list, utilization: dict) -> list:
    """Merge utilization data into license records."""
    matched_count = 0
    unmatched_licenses = []
    ...
    for lic in licenses:
        ...
        if key in utilization:
            ...
            # Add utilization fields with _USED suffix (CSV has lowercase, convert to uppercase)
            for field, value in util.items():
                if field not in ['cluster', 'msid', 'system_number', 'as_of_date']:
                    try:
                        # Store with uppercase field name + _USED suffix
                        lic[f'{field.upper()}_USED'] = int(value) if value else 0
                    except:
                        lic[f'{field.upper()}_USED'] = 0
            
            # Special mapping: CSV uses CONSOLE_STATIONS, XML uses DIRECTSTATIONS
            # Copy CONSOLE_STATIONS_USED to DIRECTSTATIONS_USED for transfer candidate logic
            if 'CONSOLE_STATIONS_USED' in lic:
                lic['DIRECTSTATIONS_USED'] = lic['CONSOLE_STATIONS_USED']
            
            matched_count += 1
        else:
            unmatched_licenses.append(f"{cluster}/{msid}/{system_number}")
    
    print(f"✓ Merged utilization data for {matched_count}/{len(licenses)} systems")
    if unmatched_licenses:
        print(f"  Note: {len(unmatched_licenses)} systems without utilization data:")
        for unmatched in unmatched_licenses[:10]:  # Show first 10
            print(f"    - {unmatched}")
        if len(unmatched_licenses) > 10:
            print(f"    ... and {len(unmatched_licenses) - 10} more")
...
def main():
    ...
    utilization = load_utilization_data()
    if utilization:
        licenses = merge_utilization_data(licenses, utilization)
        print(f"✓ Merged utilization data for {len(utilization)} systems")
    print()
    
    # Step 2: Calculate costs
    print("[2/5] Calculating costs...")
    
    # Check for MPC 2026 override catalog
    override_catalog_path = Path('../config/cost_catalog_mpc_2026.json')
    if override_catalog_path.exists():
        print("  Using MPC 2026 pricing overrides...")
        cost_calc = CostCalculator(
            catalog_path='../config/cost_catalog.json',
            placeholder_cost=thresholds['placeholder_cost'],
            override_catalog_path=str(override_catalog_path)
        )
    else:
        print("  Using Honeywell 2021-2022 baseline pricing...")
        cost_calc = CostCalculator(
            catalog_path='../config/cost_catalog.json',
            placeholder_cost=thresholds['placeholder_cost']
        )
